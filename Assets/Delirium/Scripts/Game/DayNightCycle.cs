...
namespace Developers.Mathias
{
	public class DayNightCycle : MonoBehaviour
	{
		...
		private void UpdateSun()
		{
			...
			{
				intensityPercent = 0.0f;
			}
			else if (currentTimeOfDay <= 0.25f)
			{
				intensityPercent = Mathf.Clamp01((currentTimeOfDay - 0.23f) * (1 / 0.02f));
			}
			else if (currentTimeOfDay >= 0.73f)
			{
				intensityPercent = Mathf.Clamp01(1 - (currentTimeOfDay - 0.73f) * (1 / 0.02f));
			}
